class DataManager {
    ...
    static async requestApi(url, {method = "GET", data = null, isFormData = false} = {}) {
        ...
        if (!isFormData && method !== "GET" && method !== "HEAD") {
            ...
            data = JSON.stringify({...data, _token: this.getToken("X-CSRF-TOKEN"), _xtoken: this.getToken("X-Token")});
        } else if (isFormData && method !== "GET") {
            data.append('_token', this.getToken("X-CSRF-TOKEN"));
            data.append('_xtoken', this.getToken("X-Token"));
        } else if (method === "GET") {
            headers.append("Content-Type", "application/json");
            headers.append("X-CSRF-TOKEN", this.getToken("X-CSRF-TOKEN"));
            headers.append("X-Token", this.getToken("X-Token"));
        }
    ...
    static openLoading() {
        Swal.fire({
            title: 'Please Wait...',
            text: 'We are processing your request, this may take a moment.',
            icon: 'info',
