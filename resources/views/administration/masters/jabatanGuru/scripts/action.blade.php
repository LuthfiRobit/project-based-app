<script>
    ...
    $('#example').on('click', '.dropdown-item', function() {
        ...
        var url = '{{ route('master.jabatan-guru.show', ':id') }}'.replace(':id', dataId);
            ...
            case 'action_show':
                AjaxHandler.sendGetRequest(url, function(response) {
                    if (response.status === 200 && response.data) {
                        $('#detail_nama_jabatan').text(response.data.nama_jabatan || 'N/A');
                        $('#detail_deskripsi').text(response.data.deskripsi || 'N/A');
                        $('#detail_status').text(response.data.status ? response.data.status : 'N/A');
            ...
            case 'action_edit':
                AjaxHandler.sendGetRequest(url, function(response) {
                    if (response.status === 200 && response.data) {
                        ...
                        $('#editForm').attr('data-id', response.data.id_jabatan);
                        $('.selectpicker').selectpicker(
